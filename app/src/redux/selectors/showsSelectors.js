...
const sortShowsByType = (type)=>{
    return createSelector([selectAllShows],(shows)=>{
        if(type === HIGH_RATED_SHOWS){
            ...
            return [...shows].sort((a,b)=>new Date(b.premiered) - new Date(a.premiered));
        }
        return [...shows];
    });
};
export const selectorSortedHighRatedShows =  sortShowsByType(HIGH_RATED_SHOWS);
export const selectorSortedNewShows =  sortShowsByType(NEW_SHOWS);
// search results
...
export const selectShowMetadata = createSelector([selectShowDetail],(showDetail)=>{
...
export const selectShowDescription = createSelector([selectShowDetail],(showDetail) => showDetail?.summary);
export const selectShowsSeasons = createSelector([selectShowDetail],(showDetail)=>showDetail?._embedded?.seasons);
export const selectShowCast = createSelector(
    [selectShowDetail],
    (showDetail) => showDetail?._embedded?.cast
  );
export const selectShowEpisodes = createSelector([selectShowDetail],(showDetail) => showDetail?._embedded?.episodes);
